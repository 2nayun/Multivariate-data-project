...
#[7] 정준 점수 플롯
plot(fifa_score[,c(3,5)],pch=1,col="blue", ,xlim=c(-2.5,2.5),ylim=c(-2,2))
abline(v=0,h=0,lty=2)
text(fifa_score[,c(3,5)],labels=fifa$Name,pos=3,col="red", cex=0.8)
legend('topleft', legend = paste("corr =",format(fifa_cc$cor[1], digits = 3)))

plot(fifa_score[,c(3,5)],pch=1,col="blue", ,xlim=c(-2.5,2.5),ylim=c(-2,2))
abline(v=0,h=0,lty=2)
text(fifa_score[,c(3,5)],labels=1:nrow(fifa_score),pos=3,col="red", cex=0.8)
legend('topleft', legend = paste("corr =",format(fifa_cc$cor[1], digits = 3)))
...
windows()
plot(fifa_loading[,1:2],pch=1,col="red", xlim=c(-1.2,0.1),ylim=c(-0.1,0.7))
abline(v=0,h=0,lty=2)
text(fifa_loading[,1:2],labels=rownames(fifa_loading),pos=3,col="blue")
